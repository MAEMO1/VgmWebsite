...
def compose():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            db.session.rollback()
            flash(_('Error sending message.'), 'error')
...
def view_message(message_id):
    ...
    if message.recipient_id != current_user.id and message.sender_id != current_user.id:
        flash(_('You do not have permission to view this message.'), 'error')
        return redirect(url_for('messages.inbox'))
...
def delete_message(message_id):
    ...
    if message.recipient_id != current_user.id and message.sender_id != current_user.id:
        flash(_('You do not have permission to delete this message.'), 'error')
        return redirect(url_for('messages.inbox'))
    ...
    try:
        ...
        db.session.rollback()
        flash(_('Error deleting message.'), 'error')
