    ...
    try:
        ...
        try:
            ...
            query = IfterEvent.query\
                ...
                .filter(IfterEvent.date.between(period_start, period_end))
            ...
            # Get prayer times
            logger.debug("Fetching prayer times...")
            prayer_times = prayer_service.get_prayer_times_range(
                start_date=period_start,
                end_date=period_end
            )
            logger.info(f"Retrieved prayer times for {len(prayer_times) if prayer_times else 0} days")
...
def index():
    ...
    try:
        ...
        programs = RamadanProgram.query\
            .filter(RamadanProgram.start_date >= datetime.now())\
            .order_by(RamadanProgram.start_date)\
            .limit(3)\
            .all()
...
def debug_iftar_2(): #Renamed to avoid conflict
    ...
    try:
        ...
        try:
            #calendar = IfterCalendar(db) #Removed IfterCalendar usage
            events = IfterEvent.query.all() #Directly query IfterEvents for debug
        ...
        try:
            ...
            if time_type == 'prayer':
                try:
                    ...
                    prayer_service = PrayerService() # Corrected to use PrayerService
                    prayer_times = prayer_service.get_prayer_times_batch(
