...
@routes.route('/ramadan/iftar/<int:iftar_id>/edit', methods=['GET', 'POST'])
@login_required
def edit_iftar(iftar_id):
    ...
    mosques = User.query.filter_by(user_type='mosque', is_verified=True).all() if current_user.is_admin else None
    return render_template('ramadan/edit_iftar.html', iftar=iftar, mosques=mosques)

@routes.route('/ramadan/generate-calendar', methods=['GET', 'POST'])
@login_required
def generate_calendar():
    ...
    if request.method == 'POST':
        try:
            ...
            # Query events
            events = IfterEvent.query.filter(
                IfterEvent.date >= month_start,
                IfterEvent.date <= month_end
            ).order_by(IfterEvent.date, IfterEvent.start_time).all()

            print(f"Found {len(events)} events for calendar generation")

            # Format events for Canva
            formatted_events = []
            for event in events:
                formatted_events.append({
                    'date': event.date.strftime('%Y-%m-%d'),
                    'title': f"Iftar @ {event.mosque.mosque_name}",
                    'time':event.start_time.strftime('%H:%M'),
                    'location': f"{event.mosque.mosque_street} {event.mosque.mosque_number}",
                    'type': event.recurrence_type if event.is_recurring else 'single',
                    'is_family_friendly': event.is_family_friendly
                })

            print(f"Formatted {len(formatted_events)} events for Canva")

            # Create calendar design
            design = canva_client.create_calendar_design(
                title=title,
            ...
            if design:
                print(f"Successfully created Canva design: {design.get('id')}")
                flash(_('Kalender succesvol gegenereerd! U kunt deze nu bewerken in Canva.'), 'success')
                return redirect(design.get('editUrl', url_for('main.iftar_map')))
            else:
                print("Failed to create Canva design")
                flash(_('Er is een fout opgetreden bij het genereren van de kalender.'), 'error')

        except Exception as e:
            print(f"Error generating calendar: {str(e)}")
            flash(_('Er is een fout opgetreden bij het genereren van de kalender.'), 'error')

    return render_template('ramadan/generate_calendar.html', current_date=current_date)